...
for file in files:
    ...
    # Extract old modal
    # In caja-index, it's anular-venta-modal
    modal_name = "anular-venta-modal" if "caja-index" in file else "confirmar-eliminar"
    
    # Regex to find old modal: <flux:modal ... name="modal_name" ...> ... </flux:modal>
    # Note: the old modal used :closable="false"
    old_match = re.search(r'(<flux:modal[^>]*name="' + modal_name + r'".*?</flux:modal>)', old_content, re.DOTALL)
    
    # Regex to find new modal
