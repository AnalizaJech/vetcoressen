<script>
    ...
    window.vetDashboardCharts = window.vetDashboardCharts || {
        atenciones: null,
        ingresos: null
    };
    ...
    window.renderAtencionesDashboardChart = function(canvas, data) {
        if (!canvas || typeof Chart === 'undefined') return;
        if (window.vetDashboardCharts.atenciones) {
            try { window.vetDashboardCharts.atenciones.destroy(); } catch(e) {}
            window.vetDashboardCharts.atenciones = null;
        }
        ...
        window.vetDashboardCharts.atenciones = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
                datasets: [
    ...
    window.renderIngresosDashboardChart = function(canvas, data) {
        if (!canvas || typeof Chart === 'undefined') return;
        if (window.vetDashboardCharts.ingresos) {
            try { window.vetDashboardCharts.ingresos.destroy(); } catch(e) {}
            window.vetDashboardCharts.ingresos = null;
        }
        const existing = Chart.getChart(canvas);
        if (existing) existing.destroy();
        ...
        window.vetDashboardCharts.ingresos = new Chart(ctx, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [{
    ...
    window.atencionesChartComponent = function() {
        return {
            ...
            initChart(initialData) {
                ...
                const waitForChart = () => {
                    ...
                    if (typeof Chart !== 'undefined' && ctx) {
                        window.renderAtencionesDashboardChart(ctx, this.cachedData);
                    } else {
                ...
                window.addEventListener('dashboard-charts-updated', (e) => {
                    ...
                    const atenciones = detail.atenciones || (e.detail && e.detail.atenciones);
                    if (atenciones) {
                        this.updateData(atenciones);
                    }
                });
            },
            ...
            updateData(data) {
                ...
                if (!ctx) return;
                if (typeof Chart !== 'undefined') {
                    window.renderAtencionesDashboardChart(ctx, data);
                }
            }
        };
    };
    ...
    window.ingresosChartComponent = function() {
        return {
            ...
            initChart(initialData) {
                ...
                const waitForChart = () => {
                    ...
                    if (typeof Chart !== 'undefined' && canvas) {
                        window.renderIngresosDashboardChart(canvas, this.cachedData);
                    } else {
                        setTimeout(waitForChart, 50);
                    }
                };
                ...
                window.addEventListener('dashboard-charts-updated', (e) => {
                    const detail = Array.isArray(e.detail) ? e.detail[0] : (e.detail || {});
                    const ingresos = detail.ingresos || (e.detail && e.detail.ingresos);
                    if (ingresos) {
                        this.updateData(ingresos);
                    }
                });
            },
            ...
            updateData(data) {
                ...
                if (!canvas) return;
                if (typeof Chart !== 'undefined') {
                    window.renderIngresosDashboardChart(canvas, data);
                }
            }
        };
    };
...
<div>
    ...
    <div class="space-y-6">
        ...
        <div class="vc-panel flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            {{-- Filtro Global de Periodo + Botones de Acción Rápida --}}
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="kpi-card kpi-card--emerald shadow-sm">
                <div class="flex justify-between items-start mb-3">
                    <span class="text-xs font-semibold text-zinc-500 uppercase tracking-wider" x-text="$store.i18n.t('dashboard.salesPeriod') || ($store.i18n.locale === 'en' ? 'Period Revenue' : 'Ingresos del Período')">Ingresos del Período</span>
                ...
                <h3 class="text-2xl md:text-3xl font-extrabold text-zinc-900 dark:text-zinc-100 tracking-tight font-display mb-1">
                    {{ $simboloMoneda }} {{ number_format($ventasHoy, 2) }}
                </h3>
            ...
            {{-- KPI 2: Atenciones del Periodo --}}
            <div class="kpi-card kpi-card--blue shadow-sm">
                <div class="flex justify-between items-start mb-3">
                    <span class="text-xs font-semibold text-zinc-500 uppercase tracking-wider" x-text="$store.i18n.t('dashboard.attentionsPeriod') || ($store.i18n.locale === 'en' ? 'Period Appointments' : 'Atenciones del Período')">Atenciones del Período</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="vc-panel flex flex-col justify-between"
                 ...
                 @dashboard-charts-updated.window="const d = Array.isArray($event.detail) ? $event.detail[0] : ($event.detail || {}); if (d && d.atenciones) updateData(d.atenciones)"
            >
                ...
                <div class="relative h-64 w-full" style="min-height: 256px; height: 256px;" wire:ignore>
                    <canvas id="atencionesChart" style="display: block; width: 100%; height: 256px;"></canvas>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="vc-panel flex flex-col justify-between"
                 ...
                 @dashboard-charts-updated.window="const d = Array.isArray($event.detail) ? $event.detail[0] : ($event.detail || {}); if (d && d.ingresos) updateData(d.ingresos)"
            >
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                    <div>
                        <h2 class="text-base font-extrabold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                ...
                <div class="relative h-64 w-full" style="min-height: 256px; height: 256px;" wire:ignore>
                    <canvas id="ingresosChart" style="display: block; width: 100%; height: 256px;"></canvas>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 min-w-0">
            ...
            <div class="vc-panel flex flex-col justify-between overflow-hidden min-w-0">
                <div class="mb-4">
                    <div class="flex items-center justify-between gap-2">
                        <h2 class="text-base font-extrabold text-zinc-900 dark:text-zinc-100 flex items-center gap-2 min-w-0">
                            <span class="material-symbols-outlined text-indigo-500 shrink-0">psychology</span>
                            <span class="truncate" x-text="$store.i18n.t('dashboard.topDiseasesTitle') || 'Enfermedades Frecuentes & Predicción'">Enfermedades Frecuentes & Predicción</span>
                        </h2>
                        <span class="badge badge-emerald text-[10px] shrink-0" x-text="$store.i18n.t('dashboard.aiPredictive') || 'IA Predictiva'">IA Predictiva</span>
                    </div>
                    <p class="text-[11px] text-zinc-400 mt-0.5 break-words" x-text="$store.i18n.t('dashboard.topDiseasesSubtitle') || 'Proyección de demanda de fármacos e insumos para abastecimiento preventivo'">
                        Proyección de demanda de fármacos e insumos para abastecimiento preventivo
                    </p>
                </div>

                <div class="space-y-3 overflow-y-auto overflow-x-hidden max-h-96 pr-1">
                    @forelse($enfermedadesTop as $enf)
                        <div class="p-3.5 rounded-xl border border-zinc-100 dark:border-zinc-800/80 bg-zinc-50/50 dark:bg-zinc-800/30 space-y-2 min-w-0 overflow-hidden">
                            <div class="flex items-center justify-between gap-2">
                                <h4 class="text-xs font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-1.5 min-w-0">
                                    <span class="material-symbols-outlined text-[15px] text-indigo-500 shrink-0">stethoscope</span>
                            ...
                            <div class="p-2 rounded-lg bg-white/70 dark:bg-zinc-900/60 border border-zinc-200/50 dark:border-zinc-800 text-[11px] text-zinc-600 dark:text-zinc-300 flex items-start gap-1.5 min-w-0 overflow-hidden">
                                ...
                                <div class="min-w-0 break-words flex-1">
                                    ...
                                    <span class="break-words">{{ $enf['medicamentos'] }}</span>
                                </div>
                            </div>
                        </div>
                    @empty
            ...
            <div class="vc-panel flex flex-col justify-between overflow-hidden min-w-0">
                <div class="mb-4">
                    <div class="flex items-center justify-between gap-2">
                        <h2 class="text-base font-extrabold text-zinc-900 dark:text-zinc-100 flex items-center gap-2 min-w-0">
                            <span class="material-symbols-outlined text-teal-500 shrink-0">ecg_heart</span>
                            <span class="truncate" x-text="$store.i18n.t('dashboard.topSymptomsTitle') || 'Síntomas Recurrentes & Equipamiento'">Síntomas Recurrentes & Equipamiento</span>
                        </h2>
                        <span class="badge badge-blue text-[10px] shrink-0" x-text="$store.i18n.t('dashboard.diagnostics') || 'Diagnóstico'">Diagnóstico</span>
                    </div>
                    <p class="text-[11px] text-zinc-400 mt-0.5 break-words" x-text="$store.i18n.t('dashboard.topSymptomsSubtitle') || 'Kits de diagnóstico, pruebas laboratoriales y equipos médicos sugeridos'">
                ...
                <div class="space-y-3 overflow-y-auto overflow-x-hidden max-h-96 pr-1">
                    @forelse($sintomasTop as $sintoma)
                        <div class="p-3.5 rounded-xl border border-zinc-100 dark:border-zinc-800/80 bg-zinc-50/50 dark:bg-zinc-800/30 space-y-2 min-w-0 overflow-hidden">
                            <div class="flex items-center justify-between gap-2">
                                <h4 class="text-xs font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-1.5 min-w-0">
                                    <span class="material-symbols-outlined text-[15px] text-teal-500 shrink-0">healing</span>
                                    <span class="break-words line-clamp-2 min-w-0">{{ $sintoma['nombre'] }}</span>
                                </h4>
                            ...
                            <div class="p-2 rounded-lg bg-white/70 dark:bg-zinc-900/60 border border-zinc-200/50 dark:border-zinc-800 text-[11px] text-zinc-600 dark:text-zinc-300 flex items-start gap-1.5 min-w-0 overflow-hidden">
                                ...
                                <div class="min-w-0 break-words flex-1">
                                    ...
                                    <span class="break-words">{{ $sintoma['insumos'] }}</span>
                                </div>
                            </div>
                        </div>
                    @empty
